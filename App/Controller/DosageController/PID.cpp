...
T PIDController<T>::tick()
{
  ...
  if(enabled)
  {
    ...
    _peffect = (error * _p);
    _ieffect = (integralCumulation * _i);
    _deffect = (cycleDerivative * _d);
 ...
 */
template <class T>
void PIDController<T>::setPIDSource(std::function<T()> cb)
{
  _pidSource = std::move(cb);
}
 ...
 */
template <class T>
void PIDController<T>::setPIDOutput(std::function<void(T)> cb)
{
  _pidOutput = std::move(cb);
}
 ...
 */
template <class T>
void PIDController<T>::registerTimeFunction(uint32_t (*getSystemTime)())
{
