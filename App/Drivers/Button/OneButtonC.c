...
void OB_SetPressMs(OneButton_t* btn, uint64_t ms) {
    if (!btn) return;
    btn->press_ms = ms;
}
    ...
    switch (btn->state) {
        ...
        case OCS_DOWN: // waiting for level to become inactive.
            if (!activeLevel) {
                ...
                if (btn->longPressStartFunc) {
                    btn->longPressStartFunc(btn->usrData);
                }
        ...
        case OCS_COUNT: // dobounce time is over, count clicks
            if (activeLevel) {
                ...
                if (btn->nClicks == 1) {
                    if (btn->clickFunc) {
                        btn->clickFunc(btn->usrData);
                    }
                } else if (btn->nClicks == 2) {
                    if (btn->doubleClickFunc) {
                        btn->doubleClickFunc(btn->usrData);
                    }
                } else {
                    if (btn->multiClickFunc) {
                        btn->multiClickFunc(btn->usrData);
                    }
                }
                OB_Reset(btn);
            }
            ...
            if (!activeLevel) {
                ...
                if (btn->longPressIntervalMs > 0 && 
                    ...
                    if (btn->duringLongPressFunc) {
                        btn->duringLongPressFunc(btn->usrData);
                    }
                    btn->lastDuringLongPressTime = btn->now;
                }
            }
        ...
        case OCS_PRESSEND: // unknown state detected -> reset state machine
            if (btn->longPressStopFunc) {
                btn->longPressStopFunc(btn->usrData);
            }
            OB_Reset(btn);
            break;
